...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function addTask(name, subject, description, date, type, reminderInterval, groupMembers) {
        ...
        if (user) {
            await updateTaskCounts(user.uid, 0, 1); // Update pending tasks
        } else {
            console.error("User is not authenticated");
            return; // Prevent the operation if the user is not logged in
        }
    }
    ...
    deleteTaskButton.onclick = async function() {
        ...
        if (user) {
            const userId = user.uid;
            const taskRef = doc(db, "tasks", eventId); // Reference the task document by ID
